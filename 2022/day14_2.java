...
public class day14_2 {
    ...
    public static void main(String[] args) {
        ...
        for (String line : FileUtils.readLines("2022/day14tmp.txt")) {
        ...
        while (true) {
            ...
            while (true) {
                if (!solidityMap.containsKey(new Position2D(sandPiece.x, sandPiece.y+1)) && sandPiece.y+1<maxY+2) {
                    sandPiece.y += 1;
                } else if (!solidityMap.containsKey(new Position2D(sandPiece.x-1, sandPiece.y+1)) && sandPiece.y+1<maxY+2) {
                    sandPiece.x -= 1;
                    sandPiece.y += 1;
                } else if (!solidityMap.containsKey(new Position2D(sandPiece.x+1, sandPiece.y+1)) && sandPiece.y+1<maxY+2) {
                    sandPiece.x += 1;
            ...
            if (sandPiece.equals(new Position2D(500, 0))) {
                break;
            }
        }
        System.out.println("%s sand units could settle before the source was covered.".formatted(settledSand));
    }
